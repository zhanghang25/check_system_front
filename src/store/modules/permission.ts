    ...
    async getRoutes(data: { userId: number; roleId: number }) {
      try {
        if (getMenuListByRoleId) {
          ...
          let arrRoute = [] as Array<any>
          if (parseInt(res.data.roleId) == 1) {
            arrRoute = adminRoutesMy
          } else if (parseInt(res.data.roleId) == 2) {
            arrRoute = cheJianRoutes
          } else if (parseInt(res.data.roleId) == 3) {
            arrRoute = duanShangRoutes
          }

          return generatorRoutes(arrRoute)
        } else {
